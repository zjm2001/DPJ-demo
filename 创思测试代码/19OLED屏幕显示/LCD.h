...
//可传入变量
void LCD_bl(unsigned char x, unsigned char y, unsigned int num)
{
    char str[6];  // 存放转换后的数字字符串，最大长度为5位数加空字符
    // 将数字转换成字符串
    sprintf(str, "%d", num);
    // 调用LCD_P8x16Str()函数显示字符串
...
void LCD_welcome()
{
    ...
    LCD_P8x16Str(16, 3, "wd:");
    LCD_P8x16Str(70, 3, "sd:");
    LCD_P8x16Str(16, 5, "mq:");
    LCD_P8x16Str(70, 5, "gz:");
}